...
- **Stortinget issue sync**: `GET /api/cron/sync-issues` med header `x-cron-secret` (`CRON_SECRET` env) — bulk upsert `stortinget_issues` (100/chunk) via `lib/stortinget-sync.ts`. **n8n schedule** [`Folkets Stemme – App cron`](https://n8n.heyklever.app/workflow/rwiy05sitrv5QDbQ) (ID `rwiy05sitrv5QDbQ`, kilde `workflows/n8n/app-cron.workflow.ts`, daglig 03:00 m.fl.), ikke Vercel Cron (krever Pro). `GET /api/forum/context-search` søker DB først, faller tilbake til live Stortinget-API når sparse.
